...
class GPIB():
    ...
    def open(self):
        ...
        except serial.SerialException, e:       # usually an error with trying to open the port when in use
            print e
            self.close()
        except KeyboardInterrupt, e:            # to allow keyboard interrupt if it gets hung up
            print e
            self.close()
