        ...
        if self.unique_freqs:
            ...
            for freq in self.unique_freqs:
                ...
                if len(f) <= 3:
                    ...
                    f = f[:-6] + 'MHz'
                if 'impedance' in filename.lower():
                    disp_A = 'Impedance Amplitude'
                    unit_A = '[ohm]'
                    disp_B = 'Phase'
                    unit_B = '[rads]'
                elif 'resistance' in filename.lower():
                    disp_A = 'Resistance'
                    unit_A = '[ohm]'
                    disp_B = 'Capacitance'
                    unit_B = '[F]'
                ...
                labels.extend(['Time %s [s]' % f,
                               'Temperature A %s [K]' % f,
                               'Temperature B %s [K]' % f,
                               ('%s %s %s' % (disp_A, f, unit_A)).strip(' '),
                               ('%s %s %s' % (disp_B, f, unit_B)).strip(' '),
                               'Voltage %s [V]' % f])
                if type(self.lj_chs) == list:
                    for ii, ch in enumerate(lj_chs):
                        labels.insert(3+ii*9, 'LJ AIN%d %s [V]' % (ch, f))
                        labels.insert(4+ii*9, 'Std Dev AIN%d %s [V]' % (ch, f))
