...
class Setup_Window(Tkinter.Tk):
    ...
    def go(self):
        ...
        if self.title:
            self.filename = '%s_%s_%s_sorted' % (self.title.replace(' ', '-').replace('_', '-'), self.capID.split(' ')[0], str(time.time()).replace('.', '_'))
        else:
            self.filename = '%s_%s_sorted' % (self.capID.split(' ')[0], str(time.time()).replace('.', '_'))
        ...
        data = cap.data_file(self.path, self.filename, unique_freqs=self.frequencies, comment=self.comment)
