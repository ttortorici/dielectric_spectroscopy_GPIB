...
def update():
    global path, files, c_curves, l_curves, ct_curves, lt_curves, T_curves
...
for ii, label in enumerate(labels):
    c_curves[ii] = p1.plot(pen=(ii, 6), name=label)
    l_curves[ii] = p2.plot(pen=(ii+3, 6), name=label)
    ct_curves[ii] = p4.plot(pen=(ii, 6), name=label+' capacitance')
    ct_curves[ii+3] = p4.plot(pen=(ii+3, 6), name=label+' loss')
...
for ii, label in enumerate(labels):
    T_curves[ii] = p3.plot(pen=(ii, 2), name=label)
