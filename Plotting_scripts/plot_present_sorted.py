...
class Setup_Window(Tkinter.Tk):
    ...
    def __init__(self):
        ...
        self.title('Plot Present Sorted')
        ...
        if len(self.files_to_use) > 1:
            for ii, f in enumerate(self.files_to_use[1:]):
                try:
                    ...
                    pass
        self.timestamps = np.concatenate(((data[:, 0] - data[0, 0]) / self.time_factor,
                                          (data[:, 7] - data[0, 7]) / self.time_factor,
                                          (data[:, 14] - data[0, 14]) / self.time_factor))
        self.temperature1 = np.concatenate((data[:, 1], data[:, 8], data[:, 15]))
        self.temperature2 = np.concatenate((data[:, 2], data[:, 9], data[:, 16]))
        self.capacitance = np.concatenate((data[:, 3], data[:, 10], data[:, 17]))
        self.loss = np.concatenate((data[:, 4], data[:, 11], data[:, 18]))
        self.freqs = np.concatenate((data[:, 6], data[:, 13], data[:, 20]))
...
class Plot:
    ...
    def __init__(self, title, axes_labels, time_trim, legend_loc, plot_range):
        ...
        self.fig.subplots_adjust(left=0.14)
